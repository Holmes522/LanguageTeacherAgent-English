...
> 状态：**进行中，未完成**。预检已交付并加固；工具链已安装并固定版本；**Windows SDK 缺失导致 `cargo build` 无法链接，需提权安装**；骨架、锁文件与 CI 均未开始。
...
- [ ] **安装 Windows SDK**，使 `msvc` 变为 OK、预检 `-RequireReady` 退出码 0 —— **BLOCKED：需要管理员权限，当前 shell 非提权**。VC 工具与 `link.exe` 已由预装的 VS Community 2022（`17.11.3`，VC 工具 `14.41.34120`）提供；缺的是 SDK 的 `Include\` 与 `Lib\`。
- [ ] 建立 Tauri/React、Python AI Core、contracts 和测试目录。
- [ ] 落地 `.gitattributes`（统一 LF）、`.npmrc`、`.env.example`、`.gitleaks.toml`（`.node-version` 已完成）。
- [ ] 配置 pnpm、uv、Rust 锁文件与 GitHub Actions 最小 CI（5 个 job，Windows runner）。
- 预检现状（2026-09-19 修复后，14 项 ok=12 missing=1 unknown=1）：已就绪 Git 2.51.0 / Node v22.20.0 / pnpm 12.4.2 / uv 0.12.17 / Python 3.12 / rustc 1.98.1 / cargo 1.98.1 / Corepack 0.34.0 / winget v1.29.290 / WebView2 153.0.4234.32；**缺失 msvc（Windows SDK）**；无法判定 vbscript（需提权，且仅为 MSI 前置项）。
...
- 依赖：M00 Spec v1.2（已复核通过）+ 工具链预检（已运行）+ Windows SDK（**待提权安装**）。
