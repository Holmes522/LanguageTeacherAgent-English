...
- **密钥扫描的 3 处历史命中：现状与处理决定（2026-09-19）**
  - 来源：`55ff124`（一次误用 `git add -A` 造成的提交）把当时**仍未跟踪**的测试常量带进了 git 历史。
    未跟踪文件不在历史里，所以此前的扫描一直是 0——问题是被那次误提交"制造"出来的。
  - 现已落地：`.gitleaks.toml` 有一条**范围极窄**的例外（`condition = "AND"` + 单一 commit SHA
    `55ff124…` + 三条路径 + 单一规则 `generic-api-key`），只放宽那 3 处历史命中，
    **不放宽任何未来提交**（在别的提交里写入同样形态的字符串仍会被检出，已实测）。
    加例外后：全量历史 0 命中，`pnpm check:secrets` 退出码 0（22 commits scanned）。
  - **决定（用户 2026-09-19 确认）**：采用 **「保留例外 → squash 合并 → 删除例外」** 的组合。
    用户原本选择 (c)（不加例外、靠 squash 自然消解），而例外已由另一会话按 (b) 落地；
    经说明后用户确认保留它到合并完成。保留的理由：按 (c) 字面执行会让分支门禁在合并前一直红，
    而"按设计持续失败"的门禁会训练人忽略失败；该例外范围受控（单一 commit + 三条路径 + 单一规则）
    且自带到期条件。
  - **合并后必须做的两步**：① 删除 `.gitleaks.toml` 里那条例外；② 重跑 `pnpm check:secrets`
    确认 `main` 上仍为 0 命中（`main` 的历史不含 `55ff124`，所以不需要任何例外）。
    该收尾项已登记在 `tasks/todo.md`。
  - **合并 PR #1 时必须用 squash，不能用普通 merge commit。** 若用普通合并，`55ff124` 及其 blob
    会进入 `main` 的历史，本问题会重新出现。
  - 合并后应删除该例外并重跑 `pnpm check:secrets`，确认 `main` 上仍为 0。
  - 已知小尾巴：`services/ai-core/tests/test_sidecar_http.py:318` 仍含一个 `sk-live-` 前缀的测试值
    （低熵，未被规则命中，因此不影响门禁）；是否统一成 `engm-test-sentinel-*` 由该文件的作者决定。
