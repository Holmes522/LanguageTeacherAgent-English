...
- **密钥扫描的 3 处历史命中：现状与处理决定（2026-09-19）**
  ...
  - **与用户选择的差异**：用户选择的是 **(c) 靠 squash 合并自然消解**，原则上不加例外；
    而该例外已由另一会话落地（属方案 b）。两者需要调和，我的建议是
    **保留该例外直到 squash 合并完成**：一个"按设计持续失败"的门禁会训练人忽略失败，
    而这条例外自带移除条件，其第 1 条正是"该分支被 squash 合并进 main"。
