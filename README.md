...
> **当前状态：Pre-alpha（开发早期）**
> EngMentor 还**不能安装、不能下载分发**。但**从源码运行已经能真正用起来一次**：桌面壳能启动、可以在本机安全配置 DeepSeek 凭据、可以和模型完成一次真实的流式对话。
> **仍没有任何教学功能**：没有查词、语法诊断、评分，没有知识库、检索与来源引用，也没有文档导入。界面里只有「对话」与「设置」两页，界面上的文字会如实说明这一点。
...
**Pre-alpha：不能安装分发，但可以从源码运行并完成一次真实对话。**

| 项目 | 状态 |
|---|---|
| 产品与技术方案（统一方案） | 已完成并存档 |
| T000 产品边界与技术决策（Q1–Q8） | 已完成，逐项由负责人确认 |
| M00 工程设计 Spec（Monorepo、契约、CI） | v1.5，已复核通过 |
...
| 版本化契约内容（JSON Schema 与生成类型） | **已完成**（T011）：6 个契约来源、TS/Python 生成物、三方一致性 32 条用例 |
...
| **设置与凭据存储**（T012） | **可用**：密钥存入 Windows 凭据管理器，界面读不回；模型 id、服务地址、上云同意可配 |
| **本地 AI Core（Sidecar）** | **可用**：只监听 `127.0.0.1` 随机端口，每次启动随机 token，只由主进程访问 |
| **一次真实流式对话**（M03 最小版本） | **可用**：消息发给 DeepSeek，回答逐字渲染。**无知识库、无引用、无评分** |
| 教学功能（查词 / 语法 / 评分 / 导入 / 题库） | **一个都没有实现** |
| Windows 安装包 | **尚未创建**（因此还不能给别人用） |
| 界面截图 | 尚未提供（界面可用，但截图属发布材料，等功能稳定后再补） |

**能跑起来的现在有两件事**：一是开发流程（安装依赖、静态检查、类型检查、测试、构建），
二是**一条最小可用链路** —— 配置凭据后可以真实地问答。除此之外的教学能力仍然为零。

**限制要知道**：目前只能在**你自己这台机器上、从源码**运行。Sidecar 还没有打包，
所以不能分发给别人（见「已知限制」）。
...
> **本指南中标注"可用"的小节是已实现并验证的操作步骤；标注"待实现"的仍是结构占位**，只说明将来会写什么、依赖哪个模块，不含可执行步骤。功能真正交付时，对应小节会在同一次改动中改写成可照做的步骤，并附上实际界面截图。

### 1. 安装与升级

**待实现。** 将说明：从何处获取 Windows 安装包；系统要求；安装步骤；升级方式与升级时的数据处理；如何确认安装包来源可信（签名校验）。依赖：M01 桌面壳与发布流程（T050）。

> 现在要运行产品，只能**从源码启动**（见「[从源码运行](#从源码运行目前唯一的方式)」）。还不存在安装包，也不能分发给别人。

### 2. 首次启动与隐私设置（**可用**）

EngMentor 目前没有安装包，启动方式见「[从源码运行](#从源码运行目前唯一的方式)」。启动后默认停在「对话」页。

**首次想真正提问，需要做三件事**（顺序无所谓，但都要做）：

1. 在「设置」里保存 DeepSeek API Key（见下一节）。
2. 在「设置 → 内容上云」勾选同意。
3. 回到「对话」页（可先点「检查 Sidecar」确认链路就绪）。

**关于"哪些内容会离开本机"** —— 这是本产品最需要你知道的一节：

- **同意开关默认关闭。** 没勾选时，提问根本不会发往云端模型 —— 拦截发生在桌面主进程里，
  不是只在界面上灰掉按钮。所以即使界面被绕过，也不会发出去。
- **勾选之后**才会把内容发给 DeepSeek。要发送的是：**你当前输入的问题**，以及（未来）回答所需的
  少量资料片段。当前版本还没有知识库，因此**只发送你输入的文字**。
- **不会发送**：你的整份文档、知识库全文、本地数据库，以及你的 API Key
  （密钥只在你的电脑与 DeepSeek 之间使用，作为请求头发出，不进入对话内容）。
- **随时可以关掉。** 取消勾选后立即生效，之后的提问不会再发往云端。

**你的数据存在哪里**（全部在仓库之外的 `%APPDATA%\EngMentor\`）：

| 文件 | 内容 | 是否含密钥 |
|---|---|---|
| `settings.json` | 上云同意开关、模型 id、服务地址 | **不含**（有测试断言只有这三个键） |
| Windows 凭据管理器中的 `com.engmentor.desktop` | DeepSeek API Key | 就是密钥本体，由操作系统加密保管 |

### 3. 配置 DeepSeek（**可用**）

1. 打开「设置」页。
2. 在「模型服务」区域的 **DeepSeek API Key** 输入框里粘贴你的密钥（形如 `sk-…`）。
3. 点「保存密钥」。看到「已保存到 Windows 凭据管理器」且状态变成 **✓ 已配置** 即为成功。
   - 输入框会**立刻被清空**，这是有意的：密钥之后不会再被显示出来。
   - EngMentor **无法把已保存的密钥读回界面** —— 没有任何命令会返回它，这不是权限设置，是接口里就没有这条路径。
   - 想换密钥：重新填一次并保存（会覆盖）。想删除：点「删除」。

**模型 id 与服务地址**：默认值是 `deepseek-flash` 与 `https://api.deepseek.com`。
如果你的账号可用的模型 id 不同，在这里改。服务地址**必须**是 `https://` 开头
（明文 http 会让密钥在链路上暴露），且不能内嵌用户名密码。改完点「保存模型设置」。

**回到「对话」页点「检查 Sidecar」**，会真的把本地 AI Core 进程拉起来并读一次健康检查：

- 成功会显示「Sidecar 已就绪（契约 v1，凭据已下发）」；
- 如果显示"未找到 AI Core 的 Python 环境"，按提示先在仓库根执行
  `uv sync --locked --project services/ai-core`。

**限制与边界**（当前版本如实说明）：

- 「检查 Sidecar」**不校验密钥是否有效** —— 它只确认本地链路通了。密钥填错时，
  错误会在第一次提问时以 `模型服务返回 HTTP 401：API Key 无效或已失效` 的形式出现。
- 关闭同意开关后仍可点「检查 Sidecar」（它不发送任何你的内容），但不能提问。
...
- **「检查 Sidecar」通过 ≠ 密钥有效。** 它只验证本机链路（进程起得来、契约版本一致）。
  密钥有问题会在提问时报 `HTTP 401`。
- **没勾同意就发不出消息。** 输入框会处于禁用状态并提示"请先在设置里配置并同意上云"。
  这是预期行为，不是故障。
- **提示"尚未配置 DeepSeek API Key"。** 去「设置」保存一次密钥。
- **提示无法启动 Sidecar / 找不到 AI Core 的 Python 环境。** 在仓库根执行
  `uv sync --locked --project services/ai-core` 后重试。
- **提示 `模型服务返回 HTTP 429`。** 这是上游限流，等一会儿再试。
- **回答中途停住且显示错误。** 当前版本对超时/断流的处理还很粗糙（只有 60 秒超时与一条可读的
  失败信息），不会自动重试。稍后重发即可。
- **不要提交你的 API Key。** 密钥只应存在于 Windows 凭据管理器里；
  **反馈问题时不要粘贴密钥，也不要粘贴完整文档或私人内容。** `.env.example` 里只有占位符。

完整的问题反馈流程依赖 M12 质量与发布，**待实现**。
...
还没有安装包，因此**只能从源码启动**。前置条件是上面那些安装命令都已跑过，并且
`uv sync --locked --project services/ai-core` 已完成（Rust 侧会直接执行
`services/ai-core/.venv/Scripts/python.exe`，找不到时会给出提示并拒绝启动）。
...
启动后的使用步骤见「[首次启动与隐私设置](#2-首次启动与隐私设置可用)」与「[配置 DeepSeek](#3-配置-deepseek可用)」。

几点如实说明：

- **这是开发模式**：`pnpm tauri dev` 会同时起 Vite 开发服务器与桌面壳，改前端代码会热更新。
- **不需要密钥也能启动**，只是不能提问（健康检查与设置页可用）。
- **Sidecar 是按需启动的**：第一次「检查 Sidecar」或第一次提问时才会拉起那个 Python 进程；
  主进程退出（含崩溃）时它会自己退出，不会留下孤儿进程。这条有集成测试真实验证过。
- **不要在没有 `dist` 的情况下单独 `cargo run`**：Tauri 在编译期嵌入前端产物，先跑 `pnpm build:web`
  或直接用 `pnpm tauri dev`。

### 集成测试（默认不跑，需要手工指定）

有 6 项 Rust↔Python 集成测试会**真的把 Sidecar 进程起起来**并做 HTTP 往返，其中一项还用 `netstat`
从进程外断言它只监听 `127.0.0.1`。它们默认是 `#[ignore]`，因为 CI 的 `rust` job 不准备 Python 环境：

```powershell
cargo test --manifest-path apps/desktop/src-tauri/Cargo.toml -- --ignored
```

前置条件是 `uv sync --locked --project services/ai-core` 已跑过。
